...
from chatbot import Comandos_shell
from operacoes import Scraping, Manager_DB, ChatBot
...
def automatico():
    ...
    rows_renov = Manager_DB.select_livros_db()
...
def chat(id_face):
    ...
    dados_db = Manager_DB.select_info_user_db(id_face)
    ...
    if(len(Scraping.livros)>0):
        ...
        for i in Scraping.livros:
            print("id: {0}  livro: {1}  vencimento: {2}".format(Scraping.livros.index(i) + 1, i,
                                                                Scraping.prazos[Scraping.livros.index(i)]))
